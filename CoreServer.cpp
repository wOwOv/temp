...
int CoreServer::Network()
{
	...
	//listen()
	lnretval = listen(ListenSock, SOMAXCONN_HINT(30000));
...
bool CoreServer::Disconnect(__int64 sessionID)
{
	...
	IOCount* check;
	InterlockedIncrement16(&SessionArray[index].ref);
	...
	{
		InterlockedDecrement16(&SessionArray[index].ref);
...
unsigned long CoreServer::GetSBufferCapacity()
{
	return SBuffer::BufPool.GetCapacity();
}

unsigned long CoreServer::GetSBufferUsingCount()
{
...
unsigned __stdcall CoreServer::WorkerThread(LPVOID arg)
{
	...
	while (1)
	{
		...
		if (myoverlapped->type == 0)
		{
			InterlockedDecrement16(&tgt->rb);
		}
		if (myoverlapped->type == 1)
		{
			InterlockedDecrement16(&tgt->sb);
		}
...
bool CoreServer::Release(SESSION* tgt)
{
	...
	IndexArray.Free(tgt->index);								//인덱스번호 반환되기 전에 OnRelease호출되어야함. 반환 후 재사용되고 재사용된 세션ID에 대해 OnRelease가 갈 수도 있으므로
	...
	if (InterlockedOr(&tgt->dFlag, 0) == 0)
	{
		if (tgt->SendQ.GetUsedSize() > 0)
		{
			if (InterlockedExchange(&tgt->sendflag, 1) == 0)
			{
				...
				InterlockedIncrement((long*)&tgt->iocount);
				InterlockedIncrement16(&tgt->sb);
				int size = 0;
				for (size; size < PACKETBOXMAX; size++)
				{
					bool check = tgt->SendQ.Dequeue(&tgt->packetbox.SBufferArray[size]);
					if (check == false)
					{
						tgt->packetbox.count = size;
						break;
					}
					wsabuf[size].buf = tgt->packetbox.SBufferArray[size]->GetReadPtr();
					wsabuf[size].len = tgt->packetbox.SBufferArray[size]->GetDataSize();
				}
				ZeroMemory(&tgt->sendio.overlapped, sizeof(tgt->sendio.overlapped));
